...
$localResultDir = Join-Path $repoRoot "ec2_test_run_from_ec2"
...
Invoke-Remote "cd $RemoteDir && source .venv/bin/activate && bash aws/run_test_download.sh"

Copy-FromRemote "$RemoteDir/ec2_test_run" $localResultDir

Write-Host "Downloaded EC2 test results to $localResultDir"
